...
class NasionalDetailFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        (activity as AppCompatActivity).supportActionBar?.title = viewModel.nasional.value?.title
        return binding.root
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when(item.itemId){
            android.R.id.home -> findNavController().navigate(R.id.action_nasionalDetailFragment_to_nasionalFragment)
        }
        return true
    }
}